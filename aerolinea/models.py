...
class Reserva(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['usuario', 'vuelo'],
                condition=models.Q(estado='activa'),
                name='unique_usuario_vuelo_activa')
        ]

    def __str__(self):
        return f"Reserva {self.codigo_reserva} - {self.usuario.nombre} ({self.vuelo.codigo})"

    def save(self, *args, **kwargs):
        """Genera un código de reserva único y asigna asiento aleatorio si no existe."""
    ...
    def generar_codigo_reserva(self):
        """Genera un código único tipo 'RES-AB1234'."""
        letras = ''.join(random.choices(string.ascii_uppercase, k=2))
        numeros = ''.join(random.choices(string.digits, k=4))
        return f"RES-{letras}{numeros}"

    def generar_asiento(self):
        """Asigna asiento aleatorio (ej. 12A, 22C)."""
...
class Tarjeta(models.Model):
    ...
    def descontar_saldo(self, monto):
        """Disminuye el saldo al hacer una compra"""
        if self.saldo >= monto:
            self.saldo -= monto
            self.save()
            return True
...
class CarritoItem(models.Model):
    ...
    def subtotal(self):
        return self.vuelo.precio * self.cantidad
